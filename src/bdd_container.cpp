...
void bdd_container::calculate_most_common_action(int timestep, sas_problem problem,
                                                 planning_logic::plan_to_cnf_map variable_map) {
    ...
    for (int i = 0; i < problem.m_operators.size(); i++) {
        ...
        LOG_MESSAGE(log_level::info) << "Found " << num_solutions_for_op << " solutions for operator "
                                     << op_info.to_string();
