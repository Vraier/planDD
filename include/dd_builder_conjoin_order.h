...
std::vector<planning_logic::logic_primitive> order_all_clauses(encoder::encoder_abstract &encoder, option_values &options);

// same as order clauses but only orders the precondition, effect and frame clauses for timeset 0
std::vector<planning_logic::logic_primitive> order_clauses_for_layer(encoder::encoder_abstract &encoder, std::string &order_string,
                                                                     int layer);
