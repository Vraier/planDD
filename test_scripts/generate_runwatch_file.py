...
def planDD_topk_command(suite_name, problem, num_plans, additional_flags):
    ...
    fd_payload = "{} --sas-file output.sas --translate {}  > fd_output.txt".format(suite_downward_path, suite_problem_path)
    suite_planDD_path = join(SUITE_TO_SCRIPT_PATH, PLANDD_PATH)
    planDD_payload = "{} --sas_file output.sas --build_bdd --linear --timesteps -1 --clause_order_custom --var_order_custom --build_order rympec:: --binary_encoding --binary_exclude_impossible --binary_variables {} --num_plans {} > output.txt".format(suite_planDD_path, additional_flags, num_plans)

    whole_command = "{} && {} && {}".format(dir_change_command, fd_payload, planDD_payload)    
    return whole_command

def planDD_topk_use_fd_command(suite_name, problem, num_plans):
    sanitized_name = util.get_sanitized_domain_description(problem["d_name"], problem["p_name"])
    suite_path = join(SCRIPT_TO_TEST_OUTPUT, suite_name, sanitized_name)
    dir_change_command = create_and_change_directory_command(suite_path)
    ...
    suite_downward_path = join(SUITE_TO_SCRIPT_PATH, DOWNWARD_PATH)
    fd_payload = "{} --sas-file output.sas {} --search \"astar(lmcut())\" > fd_output.txt".format(suite_downward_path, suite_problem_path)
    suite_planDD_path = join(SUITE_TO_SCRIPT_PATH, PLANDD_PATH)
    planDD_payload = "{} --sas_file output.sas --build_bdd --linear --timesteps -1 --clause_order_custom --var_order_custom --build_order igrymx:pec: --binary_encoding --binary_exclude_impossible --binary_variables --restart --use_fd --num_plans {} > output.txt".format(suite_planDD_path, num_plans)
...
def generate_topk_runwatch_command_file(problems, suites):
    with open("all_commands.txt", "w") as runwatch_file:
        num_commands = 0
        for planner, k in suites:
            suite_name = "{}_k{}".format(planner, k)
            for problem in problems:
                task_command = ""
                if planner == "planDD":
                    task_command = planDD_topk_command(suite_name, problem, k, "")
                elif planner == "planDDRestart":
                    task_command = planDD_topk_command(suite_name, problem, k, "--restart")
                elif planner == "planDDUseFD":
                    task_command = planDD_topk_use_fd_command(suite_name, problem, k)
                elif planner == "symk":
